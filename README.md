...
<p align="center">
  <a href="https://github.com/provii/provii-verifier/actions/workflows/ci.yml"><img src="https://img.shields.io/github/actions/workflow/status/provii/provii-verifier/ci.yml?branch=main&style=flat&label=CI" alt="CI"></a>
  ...
  <a href="https://github.com/provii/provii-verifier/actions/workflows/security-audit.yml"><img src="https://img.shields.io/github/actions/workflow/status/provii/provii-verifier/security-audit.yml?branch=main&style=flat&label=cargo%20audit" alt="Security Audit"></a>
</p>
...
provii-verifier is a Rust application compiled to WebAssembly and deployed as a Cloudflare Worker. 107 source files across 17 modules. The cryptographic core uses `bellman` for Groth16 verification on BLS12-381 and `provii-crypto-verifier` for the age proof circuit. At cold start the embedded verifying key is integrity checked via Blake2b-512, then parsed and cached for the lifetime of the isolate.
...
The `fuzz/` and `tools/key-rotation` directories have their own `Cargo.lock` files and are excluded from the workspace. Build them separately with `cargo build --manifest-path fuzz/Cargo.toml` or `cargo build --manifest-path tools/key-rotation/Cargo.toml`.
